...
impl GitModal {
    ...
    fn render_delete_branch(
        ...
        let title = if selection_count > 1 {
            format!("Delete {selection_count} selected branches?")
        } else {
            format!(
                "Delete {} branch '{}'?",
        ...
        v_flex()
            ...
            .when(selection_count > 1, |this| {
                this.child(
                    Label::new("All currently selected branches will be deleted.")
                        .size(LabelSize::Small)
                        .color(Color::Muted),
                )
            })
            .when(!is_remote && selection_count == 1, |this| {
                this.child(
                    Checkbox::new(
    ...
    fn render_uncommit_head(&self, commit_sha: &str) -> impl IntoElement {
        let short_sha = if commit_sha.len() > 12 {
            &commit_sha[..12]
        } else {
            commit_sha
        };
        v_flex()
            .gap_3()
            .child(Label::new("Uncommit latest commit?").size(LabelSize::Large))
            .child(
                Label::new(format!(
                    "This will move HEAD before commit {} and keep changes staged.",
                    short_sha
                ))
                .size(LabelSize::Small),
            )
            .child(
                Label::new("Use with caution if this commit has already been pushed.")
                    .size(LabelSize::Small)
                    .color(Color::Warning),
            )
    }
...
impl Render for GitModal {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        let is_conflict_action = matches!(
            self.action,
            ModalAction::CherryPickConflict { .. } | ModalAction::RevertConflict { .. }
        );
        let content = match &self.action {
            ModalAction::DeleteBranch {
                ...
                .render_delete_branch(branch_name, *is_remote, *selection_count, cx)
                .into_any_element(),
            ...
            ModalAction::UncommitHead { commit_sha } => {
                self.render_uncommit_head(commit_sha).into_any_element()
            }
        ...
        div()
            ...
            .child(
                h_flex()
                    .gap_2()
                    .justify_end()
                    .children((!is_conflict_action).then(|| {
