use editor::Editor;
use gpui::{
    App, Background, Context, Entity, Hsla, IntoElement, PathBuilder, Pixels, Point as GpuiPoint,
    Window, canvas, point, prelude::*, px, size,
};
...
use super::colors::{get_diff_colors, get_diff_colors_from_app};

trait ConnectorData {
    fn curves(&self) -> &[ConnectorCurve];
    fn line_height(&self) -> f32;
    fn left_scroll_pixels(&self) -> f32;
    fn right_scroll_pixels(&self) -> f32;
    fn left_top_origin(&self) -> f32;
    fn right_top_origin(&self) -> f32;
    fn left_bounds(&self) -> Option<gpui::Bounds<Pixels>>;
    fn right_bounds(&self) -> Option<gpui::Bounds<Pixels>>;
    fn created_bg(&self) -> Hsla;
    fn deleted_bg(&self) -> Hsla;
    fn modified_bg(&self) -> Hsla;
}
...
impl ConnectorData for ConnectorCanvasData {
    fn curves(&self) -> &[ConnectorCurve] {
        &self.curves
    }
    fn line_height(&self) -> f32 {
    ...
    fn left_scroll_pixels(&self) -> f32 {
        self.left_scroll_pixels
    }
    fn right_scroll_pixels(&self) -> f32 {
        self.right_scroll_pixels
    }
    fn left_top_origin(&self) -> f32 {
        self.left_top_origin
    }
    fn right_top_origin(&self) -> f32 {
        self.right_top_origin
    }
    fn left_bounds(&self) -> Option<gpui::Bounds<Pixels>> {
        self.left_bounds.clone()
    }
    fn right_bounds(&self) -> Option<gpui::Bounds<Pixels>> {
        self.right_bounds.clone()
    }
    fn created_bg(&self) -> Hsla {
        self.created_bg
    }
...
/// Renders curved Bézier connector lines between two editors showing diff hunks.
/// This is a standalone function that can be used by any split editor view.
pub fn render_connector_overlay(
    left_editor: Entity<Editor>,
    right_editor: Entity<Editor>,
    curves: Vec<ConnectorCurve>,
    cx: &App,
) -> impl IntoElement {
    let (deleted_bg, created_bg, modified_bg) = get_diff_colors_from_app(cx);
    ...
    canvas(
        ...
        move |bounds, data, window, _cx| {
            draw_connectors(&bounds, &data, window);
        },
    )
    .absolute()
    .size_full()
...
fn draw_connectors(bounds: &gpui::Bounds<Pixels>, data: &impl ConnectorData, window: &mut Window) {
    ...
    for curve in data.curves() {
        ...
        let has_left_visible =
            adjusted_left_bottom > header_height && adjusted_left_top < adjusted_left_bottom;
        let has_right_visible =
            adjusted_right_bottom > header_height && adjusted_right_top < adjusted_right_bottom;

        if is_visible && (has_left_visible || has_right_visible) {
            draw_connector_ribbon(
                window,
                bounds,
                adjusted_left_top,
