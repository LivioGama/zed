...
impl SplitDiffModel {
    pub fn new(left_content: String, right_content: String) -> Self {
        let empty_path = RepoPath::from_rel_path(&RelPath::new(Path::new(""), util::paths::PathStyle::Posix).unwrap().into_owned());
        Self {
            left_spec: DiffSpec::working_directory(empty_path.clone()),
            right_spec: DiffSpec::revision(empty_path, "HEAD".to_string()),
            hunks: Vec::new(),
            left_content,
            right_content,
...
impl DiffComputation {
    ...
    pub fn compute(self, cx: &mut AsyncApp) -> Task<anyhow::Result<SplitDiffModel>> {
        cx.spawn(async move |cx| {
            ...
            Ok(SplitDiffModel {
                left_spec: DiffSpec::working_directory(RepoPath::from_rel_path(&RelPath::unix(Path::new("left")).unwrap())), // TODO: proper spec
                right_spec: DiffSpec::working_directory(RepoPath::from_rel_path(&RelPath::unix(Path::new("right")).unwrap())),
                hunks,
